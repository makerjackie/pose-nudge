...
检测修复、通知权限、菜单栏状态和可靠提醒属于产品应有的基本质量。官方安装包不拆分残缺的 Free 功能集，而是让用户先完整体验，再决定是否永久解锁。

### 7 天完整试用

- 首次启动开始计时，无需账号、信用卡或联网
- 本地姿态监测与多帧校准
- 菜单栏运行、暂停与当前状态
- 通知权限诊断、测试提醒
- 系统通知、顶部悬浮提醒和屏幕柔和变暗
...
- 试用结束后继续使用完整应用
- 永久离线使用，不依赖订阅或周期性联网校验
- 最多激活 3 台设备
- 后续 OnePosture 功能更新

价格：国内 **¥39 永久版**，海外 **US$4.99 lifetime**。不做订阅，不用“修 bug”作为付费理由。¥29 仅保留为限量早期支持者价格，不作为长期标价。试用结束后暂停监控，但保留历史数据、设置、更新入口、开源说明和激活入口。

应用仍以 AGPL-3.0 完整开源。试用边界用于官方发行版本的轻量商业化，不承诺成为不可绕过的 DRM；用户仍可依法查看、构建和修改源码。
...
- `FloatingReminderAdapter`：始终置顶的顶部悬浮窗；是系统通知失效时的可靠兜底。
- `ScreenDimAdapter`：柔和屏幕变暗。
- `SoundAdapter`：声音提醒。
...
- 首次启动后 7 天内无需 token，全部功能完整可用。
- 试用到期后监控、校准、测试提醒和菜单栏启动入口使用同一权限边界。
- Pro token 可以离线验证；激活后完整功能永久可用。
